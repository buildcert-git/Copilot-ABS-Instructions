...
Storey = enclosed level above/below another enclosed level.
...
Total enclosed building area measured to internal face of external walls.
...
Exclude 18 m² only once total across the entire development.
...
Existing GFA = area of existing structures not part of this approval.

If:

Existing buildings shown but GFA unknown → return Unknown and explain.

If:

Cannot determine if existing buildings exist → return Unknown and explain.

Units must be m².

### 4 Site Area

Site Area = total lot area.

Units must be m².

If shown in hectares, convert:

`1 hectare = 10,000 m²`

### 5 Building Height

Height = vertical distance from lowest connected point to highest connected point.
...
Find vertical dimension segments along building side.
...
Plans usually label them “existing”.

If vacant site → 0

### 7 Existing Dwellings to be Demolished

Use development description first.
...
Assess existing site condition.

Dual occupancy = two equal dwellings.
